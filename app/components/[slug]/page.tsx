...
export default async function ComponentPage({ params }: Props) {
  ...
  let component;
  try {
    component = getComponent(slug);
  } catch {
    notFound();
  }

  const { meta, content } = component;
  const usedInSystems = getSystemsBySlug(meta.usedIn ?? []);
  const alternatives = getComponentsBySlug(meta.alternatives ?? []);
